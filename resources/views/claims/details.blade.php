                ...
                <table class="table table-responsive table-noborder">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><strong>Anmelder:</strong> {{ ($claim->user) ? $claim->user->name : '' }}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td>&nbsp;</td>
                            <td>&nbsp;</td>
                            <td><button class=" btn btn-{{ getClaimColor($claim) }} " type="submit">{{ getTranslation('submit') }}</button></td>
                        </tr>
                        <tr>
    ...
    @if(count($claim->images) > 0)
        <div class="row">
            @foreach($claim->images as $key => $image)
                <div class="column">
                    <img src="{{ asset('/images/'.$image->image) }}" style="width:100%; max-height: 150px;min-height: 150px;" onclick="openModal();currentSlide({{ $key }})" class="">
                </div>
            @endforeach
        </div>
